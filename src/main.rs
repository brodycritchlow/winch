...
use install_script_creation::create_install_script_file;
...
fn run_script() {
    ...
    command.arg("-Command ./install.cmd")
}
#[cfg(target_os = "linux")]
fn run_script() {
    let mut command = Command::new("./install.sh");
}
...
fn main() {
    ...
    match args.cmd {
        Some(Command::Install { package, force: _, remote_repository}) => {
            ...
            match remote_repository {
                true => {
                    if package.contains("github") {
                        ...
                        let mut result = parse(&package);
                        create_install_script_file(&mut result.0, result.1, result.2);                        
                    }
                }
                _ => {
